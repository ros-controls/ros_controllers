...
def main():
  try:
    ...
    controller_ns = controller.name 
    JOINT_NAMES = controller.claimed_resources[0].resources
    ...
    rospy.wait_for_service(controller_ns + '/query_state')
    query_state_srv = rospy.ServiceProxy(controller_ns + '/query_state', QueryTrajectoryState)
